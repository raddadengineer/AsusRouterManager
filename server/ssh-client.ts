...
export class SSHClient {
  ...
  async getSystemInfo(): Promise<any> {
    ...
    try {
      ...
      const temperature = await this.executeCommand("cat /proc/dmu/temperature 2>/dev/null || cat /tmp/cpu_temp 2>/dev/null || nvram get fanctrl_info | grep -o 'temp.*' | cut -d':' -f2 | head -1 || echo 'N/A'");
